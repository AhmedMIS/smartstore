...
if ($productCount > 0) {
	...
	if ($result[0] == 0) {
		...
		if ($opened[$i] == 0) {
			...
			<tr>
				<td width="20%"><?php if ($opened[$i] == 0){?><h1><a href="?id=inbox&read=<?php echo $id[$i] ?>">Mark as read</a></h1><?php }?><p><a href="?id=inbox&delete=<?php echo $id[$i] ?>" onclick="return confirm('Confirm Delete?');">Delete</a></p></td>
		...
		</table><br /><hr class="style-one"></hr>
    <?php               
		}
	}
}?>