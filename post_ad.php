...
if (isset($_POST['submit'])) {
	...
	if (empty($errors)) {
		...
		$message = '<h2>Your ad was successfully added.</h2>';
...
function populate(s1,s2){
	...
	if(s1.value == "Mobile & Tablets"){
		var optionArray = ["|","Mobile Phones|Mobile Phones","Tablets|Tablets","Accessories|Accessories"];
...
<?php if (!empty($errors))  foreach($errors as $error) echo"<p style='color:red;'>&#9658 ". $error ."</p>"?>
...
<table width="90%" border="1">
	<form action="" method="post" enctype="multipart/form-data">
		<tr>
			<td><h1>Title:</h1></td>
			<td><input type="text" name="title" id="title" value="<?php if (isset($title)) echo $title ?>"  maxlength="25" /></td>
		</tr>
		<tr>
			<td><h1>Price:</h1></td>
			<td><input type="text" name="price" id="price" value="<?php if (isset($price)) echo $price ?>" maxlength="7"  onkeypress="return isNumberKey(event)" /></td>
		</tr>
		<tr>
		...
		<tr>
			<td><h1>Brand:</h1></td>
			<td><input type="text" name="brand" id="brand" maxlength="15"  ></td>
		</tr>        
		<tr>
			<td><h1>Description:</h1></td>
			<td><textarea name="description" id="description" rows="5" cols="50"  onkeypress="if (this.value.length > 300) { return false; }"/><?php if (isset($description)) echo $description ?></textarea></td>
		</tr>
        <tr>
			<td><h1>Image 1:</h1></td>
